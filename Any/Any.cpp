...
public:
	PP() : p(100) {}
	PP(const PP& other) : name("拷贝构造")
	{
		std::cout << name << std::endl;
	}

	PP(PP&& other) : name("移动构造")
	{
		std::cout << name << std::endl;
	}
	...
	~PP()
	{
		std::cout << name << "-删除" << std::endl;
	}

	void test(const std::string& data, int i)
	{
		std::string str("这是个对象中的函数: [");
