...
void OctreeNode::addAgent(OctreeAgent* agent)
{
	...
	// 	}
		...
		//最小了不分了
	OctVector3 dimensions = mBoundingBox.getDimensions();
	...
	{
		OctVector3 center = mBoundingBox.min + (dimensions * 0.5f);
		...
		*/
		...
		mChildren[1]->mBoundingBox.reset(OctVector3(mBoundingBox.min.x, center.y, center.z)
			, OctVector3(center.x, mBoundingBox.max.y, mBoundingBox.max.z));

		mChildren[2]->mBoundingBox.reset(OctVector3(mBoundingBox.min.x, mBoundingBox.min.y, center.z)
			, OctVector3(center.x, center.y, mBoundingBox.max.z));

		mChildren[3]->mBoundingBox.reset(OctVector3(center.x, mBoundingBox.min.y, center.z)
			, OctVector3(mBoundingBox.max.x, center.y, mBoundingBox.max.z));

		//back 4
		mChildren[4]->mBoundingBox.reset(OctVector3(center.x, center.y, mBoundingBox.min.z)
			, OctVector3(mBoundingBox.max.x, mBoundingBox.max.y, center.z));

		mChildren[5]->mBoundingBox.reset(OctVector3(mBoundingBox.min.x, center.y, mBoundingBox.min.z)
			, OctVector3(center.x, mBoundingBox.max.y, center.z));

		mChildren[6]->mBoundingBox.reset(mBoundingBox.min, center);

		mChildren[7]->mBoundingBox.reset(OctVector3(center.x, mBoundingBox.min.y, mBoundingBox.min.z)
			, OctVector3(mBoundingBox.max.x, center.y, center.z));
	}
